...
# final velocity
rocket.vel = earth.vel + dir_r * v_r
...
for step in range(100000):
    ...
    if step % 100 == 0:
        ...
        #trajectories
        ax.plot(*zip(*[(p[0], p[1]) for p in earth_traj]), color= "deepskyblue", linewidth=1)
        ax.plot(*zip(*[(p[0], p[1]) for p in moon_traj]),color= "gray",  linewidth=1)
        ax.plot(*zip(*[(p[0], p[1]) for p in rocket_traj]),color= "red",  linewidth=1)
        
        #scatters
        ax.scatter(sun.pos[0], sun.pos[1], s=sun.radius/scale,color= "orange", label="Sun")
        ax.scatter(earth.pos[0], earth.pos[1], s=earth.radius/scale,color= "deepskyblue", label="Earth")
        ax.scatter(moon.pos[0], moon.pos[1], s=moon.radius/scale,color= "gray", label="Moon")
        ax.scatter(rocket.pos[0], rocket.pos[1], s=10,color= "red", label="Rocket")
